            ...
            switch ($item['action']) {
                case 'same':
                    if (is_array($item['value'])) {
                        ...
                        $result = $result . str_repeat('    ', $depth) . $item['property'] . ": " . valueAsString($item['value']) . "\n";
                    }
                break;
                case 'added':
                if (is_array($item['value'])) {
                    ...
                    $result = $result . str_repeat('    ', $depth - 1) . '  + ' . $item['property'] . ": " . valueAsString($item['value']) . "\n";
                }
                break;
                case 'removed':
                    if (is_array($item['value'])) {
                ...
                case 'updated':
                    if (is_array($item['value'])) {
                        $result = $result . str_repeat('    ', $depth - 1) . '  - ' . $item['property'] . ": {\n";
                        $result = $result . stylish($item['value'], $depth + 1) . str_repeat('    ', $depth) . "}\n";
                        $result = $result . str_repeat('    ', $depth - 1) . '  + ' . $item['property'] . ": " . valueAsString($item['new value']) . "\n";
                    } else {
                    $result = $result . str_repeat('    ', $depth - 1) . '  - ' . $item['property'] . ": " . valueAsString($item['value']) . "\n";    
                    $result = $result . str_repeat('    ', $depth - 1) . '  + ' . $item['property'] . ": " . valueAsString($item['new value']) . "\n";
                    }
